...
import { Profile, Social } from '../../../../assets/interfaces/profile.interfaces';
    ...
    <div class="space-y-6">
      <form [formGroup]="form" class="space-y-6">
        ...
        <div class="space-y-2">
          ...
          @if (form.get('instaIdUrl')?.invalid && form.get('instaIdUrl')?.touched) {
            <p class="text-xs text-red-400">
              @if (form.get('instaIdUrl')?.errors?.['notInstagramUrl']) {
        ...
        <div class="space-y-2">
          ...
          @if (form.get('youtubeIdUrl')?.invalid && form.get('youtubeIdUrl')?.touched) {
            <p class="text-xs text-red-400">
              @if (form.get('youtubeIdUrl')?.errors?.['notYoutubeUrl']) {
        ...
        <!-- External Links -->
        <div class="space-y-2">
          <label class="block text-sm font-medium text-neutral-300">external links</label>
          <div class="relative">
            <input
              type="url"
          ...
          @if (form.get('externalLinkUrl')?.invalid && form.get('externalLinkUrl')?.touched) {
        ...
        @for (link of additionalLinks(); track $index) {
          <div class="space-y-2">
            <div class="flex items-center justify-between">
              <label class="block text-sm font-medium text-neutral-300">additional link {{ $index + 1 }}</label>
              <button
                type="button"
...
export class SocialsSectionComponent implements OnInit {
  ...
  initializeForm() {
    this.form = this.fb.group({
      instaIdUrl: ['', [this.urlValidator.bind(this), this.instagramValidator.bind(this)]],
      youtubeIdUrl: ['', [this.urlValidator.bind(this), this.youtubeValidator.bind(this)]],
  ...
  onSave() {
    ...
    const additionalLink = this.additionalLinks().filter(link => link.trim())[0] || '';
    ...
    this.save.emit({ social: socialData });
