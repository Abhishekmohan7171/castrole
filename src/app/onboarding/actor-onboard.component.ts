...
@Component({
  ...
  template: `
    ...
    >
      ...
      <!-- Brand -->
      <h1
        class="pt-16 pb-10 text-5xl sm:text-6xl md:text-7xl font-black tracking-wider text-neutral-400 select-none"
      >
        castrole
      </h1>
      ...
      <!-- Card -->
      <div
        class="w-full max-w-xl rounded-3xl  border border-white/5 shadow-2xl shadow-black/60 px-8 py-10"
      >
        <form
          class="space-y-5"
          [formGroup]="form"
        ...
        >
          ...
          <div class="relative">
            ...
            <div
              *ngIf="
                ...
                (form.get('location')?.value || '').length > 0
              "
              class="absolute left-0 right-0 top-full mt-2 z-20"
            >
              <div
                class="bg-neutral-900/95 backdrop-blur rounded-2xl border border-white/5 shadow-xl overflow-hidden"
              >
                <ul class="max-h-60 overflow-auto divide-y divide-white/5">
                  <li
                    *ngFor="let s of locationSuggestions"
                    (click)="onSelectLocationSuggestion(s)"
                    class="px-4 py-2 hover:bg-white/5 cursor-pointer flex items-center justify-between"
                  >
          ...
          <div class="relative">
            <div class="flex gap-2">
              <select
                ...
                class="w-36 sm:w-40 bg-neutral-800/80 text-neutral-200 rounded-full px-3 py-3 ring-1 ring-white/10 focus:ring-2 focus:ring-emerald-500/50 outline-none"
              >
              ...
              <div class="relative flex-1 flex gap-2">
                <div class="relative flex-1">
                  ...
                  >
                    ...
                    >
                      <path
                        d="M22 16.92v2a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6A19.79 19.79 0 0 1 2.12 4.18 2 2 0 0 1 4.11 2h2a2 2 0 0 1 2 1.72c.12.9.33 1.77.62 2.6a2 2 0 0 1-.45 2.11l-.9.9a16 16 0 0 0 6 6l.9-.9a2 2 0 0 1 2.11-.45c.83.29 1.7.5 2.6.62A2 2 0 0 1 22 16.92Z"
                      />
                    </svg>
                  </span>
                  <input
                ...
                @if (isPhoneVerified) {
                <div
                  class="flex items-center px-3 py-3 bg-green-600/20 border border-green-500/30 text-green-400 rounded-full text-sm"
                >
                  <svg
                    class="h-4 w-4 mr-2"
                    viewBox="0 0 24 24"
                ...
                <button
                  ...
                  "
                  class="px-4 py-3 bg-emerald-600 hover:bg-emerald-700 disabled:bg-neutral-600 disabled:cursor-not-allowed text-white font-medium rounded-full transition-colors text-sm whitespace-nowrap"
          ...
          <div
            *ngIf="errorMsg"
            class="p-3 rounded-lg bg-red-900/20 border border-red-500/20 text-red-400 text-sm text-center"
          >
            {{ errorMsg }}
          </div>
...
export class ActorOnboardComponent {
  ...
  // Method to check if current phone number is verified
  checkPhoneVerification(): boolean {
    ...
    return this.otpVerificationService.isPhoneVerified(phoneNumber);
  }
  ...
  constructor() {
    ...
    this.http
      .get<Array<{ name: string; dialCode: string; flag: string }>>(
        'assets/json/country-code.json'
      )
      .subscribe({
        next: (data) => {
    ...
    this.http
      .get<Array<{ state: string; districts: string[] }>>(
        'assets/json/processed-locations.json'
      )
      .subscribe({
        next: (data) => {
  ...
  onOtpVerified() {
    console.log('OTP verified successfully in actor onboarding');
  ...
  onNext() {
    ...
    const isPhoneVerified = this.isPhoneVerified;
